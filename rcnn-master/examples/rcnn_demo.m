function cls=rcnn_demo(demo_choice, use_gpu)
...
clf;
addpath(genpath('/home/young/Desktop/ImageCaption/rcnn-master/.'));
%startup
thresh = -1;
rootpath = '/home/young/Desktop/ImageCaption/rcnn-master/';
...
switch demo_choice
  case 'PASCAL'
    ...
    rcnn_model_file = [rootpath 'data/rcnn_models/voc_2012/rcnn_model_finetuned.mat'];
    im = imread([rootpath 'examples/images/fish-bike.jpg']);
  case 'ILSVRC13'
    % Example using the ILSVRC13 fine-tuned detectors (200 classes)
    rcnn_model_file = [rootpath 'data/rcnn_models/ilsvrc2013/rcnn_model.mat']
    filename = load('filename.mat');
    im = imread(filename.name);
    %im = imread([rootpath 'examples/images/fish-bike.jpg']);
  otherwise
...
for i = 1:length(ord)
  ...
  cls{i} = rcnn_model.classes{all_dets(ord(i), 1)};
...
save('/home/young/Desktop/ImageCaption/rcnn-master/examples/result.mat', 'cls')
